...
export function CaseFiles() {
  ...
  const filter: string = 'all' // hardcode to bypass unused setFilter warning if no buttons exist

  const filtered = cases.filter((c) => {
    const matchesQuery = c.threatName.toLowerCase().includes(query.toLowerCase()) ||
  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-3">
        ...
        {filtered.length === 0 && (
          <div className="text-center text-sm text-slate py-12">No case files match your search.</div>
        )}
      </div>
    </div>
  )
